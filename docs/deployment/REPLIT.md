...
- **Deal Scenarios Table**: Added 15 missing columns to `deal_scenarios` table for proper lease calculation support: `tax_method`, `gross_cap_cost`, `total_cap_reductions`, `adjusted_cap_cost`, `depreciation`, `monthly_depreciation_charge`, `monthly_rent_charge`, `base_monthly_payment`, `monthly_tax`, `upfront_tax`, `security_deposit`, `drive_off_breakdown`, `tax_profile`, `total_of_payments`, `total_lease_cost`. Fixed SQL syntax errors in column definitions (e.g., `TEXT TYPE` → `TEXT`).
- **Email Inbox Sync**: Implemented multi-tenant email routing with `resolveDealershipFromRecipients()` helper function in `server/email-webhook-routes.ts`. Function matches incoming email recipients (to/cc/bcc) against dealership inbox addresses in `dealership_settings` table using parameterized SQL queries with `LOWER()` for case-insensitive comparison (SQL injection safe). Replaces hardcoded `dealershipId = 'default'` with proper database lookup. Emails without dealership match are gracefully skipped.
- **Showroom Page Layout**: Restructured `client/src/pages/showroom.tsx` to match "complete sticky" header pattern used in Inventory and other pages. Moved Header Actions section (date picker, search input, refresh button, add customer button) into the sticky header div, ensuring search/filters stay visible when scrolling. Fixed JSX structure error (missing closing div for sticky header container).
