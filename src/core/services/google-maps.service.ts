 ...
 *
 * Core infrastructure service for address autocomplete, validation, and geocoding.
 * Uses Google Maps Platform for accurate customer addresses needed for tax calculations.
 *
 * Features:
 * - Address autocomplete suggestions
 * - Full address validation via geocoding
 ...
 * Google Maps Service
 *
 * Provides address validation and geocoding capabilities using Google Maps Platform API.
 * This is a core infrastructure service used across multiple modules for address handling.
 */
export class GoogleMapsService {
  ...
  constructor(apiKey?: string) {
    ...
    if (!this.apiKey) {
      console.warn(
        '[GoogleMapsService] API key not configured - address features will be disabled'
      );
    }
  }
  ...
  private parseGooglePlace(place: any): ValidatedAddress {
    ...
    const street = [getComponent(['street_number'], 'short'), getComponent(['route'])]
      .filter(Boolean)
      .join(' ');
    ...
    return {
      ...
      validationStatus,
      validationMessages:
        messages.length > 0 ? messages : ['Address validated successfully'],
    };
  }
