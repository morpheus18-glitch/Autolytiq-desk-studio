 ...
 *
 * Core service for managing multi-location dealer rooftop configurations.
 ...
 * - Recommended rooftop selection based on registration state
 *
 * @module CoreServices
 */
...
export async function deleteRooftop(rooftopId: string): Promise<boolean> {
  try {
    await db.delete(rooftopConfigurations).where(eq(rooftopConfigurations.id, rooftopId));
 ...
 */
export function convertToRooftopConfig(rooftop: RooftopConfiguration): RooftopConfig {
  return {
    ...
    stateOverrides:
      (rooftop.stateOverrides as Record<
        string,
...
export function validateRooftopConfig(
  ...
  if (
    ...
    !['DEALER_STATE', 'BUYER_STATE', 'REGISTRATION_STATE'].includes(data.defaultTaxPerspective)
  ) {
...
export async function getRecommendedRooftop(
  ...
  try {
    ...
    const driveOutRooftops = active.filter((r) => supportsDriveOut(r, registrationState));
...
/**
 * Rooftop service class wrapper (for dependency injection)
 */
export class RooftopService {
  async getRooftopsByDealership(dealershipId: string): Promise<RooftopConfiguration[]> {
    return getRooftopsByDealership(dealershipId);
  }

  async getRooftopById(rooftopId: string): Promise<RooftopConfiguration | null> {
    return getRooftopById(rooftopId);
  }

  async getRooftopByIdentifier(
    dealershipId: string,
    rooftopIdent: string
  ): Promise<RooftopConfiguration | null> {
    return getRooftopByIdentifier(dealershipId, rooftopIdent);
  }

  async getPrimaryRooftop(dealershipId: string): Promise<RooftopConfiguration | null> {
    return getPrimaryRooftop(dealershipId);
  }

  async createRooftop(data: InsertRooftopConfiguration): Promise<RooftopConfiguration> {
    return createRooftop(data);
  }
  ...
  async updateRooftop(
    ...
    return updateRooftop(rooftopId, data);
  }

  async deleteRooftop(rooftopId: string): Promise<boolean> {
    return deleteRooftop(rooftopId);
  }

  convertToRooftopConfig(rooftop: RooftopConfiguration): RooftopConfig {
    return convertToRooftopConfig(rooftop);
  }

  validateRooftopConfig(
    data: Partial<InsertRooftopConfiguration>
  ): { valid: boolean; errors: string[] } {
    return validateRooftopConfig(data);
  }

  supportsDriveOut(rooftop: RooftopConfiguration, targetState: string): boolean {
    return supportsDriveOut(rooftop, targetState);
  }

  async getRecommendedRooftop(
    dealershipId: string,
    registrationState: string
  ): Promise<RooftopConfiguration | null> {
    return getRecommendedRooftop(dealershipId, registrationState);
  }
}
 ...
 * Singleton instance
 */
export const rooftopService = new RooftopService();
