 ...
 */
function isFileApproved(filePath) {
  ...
  for (const forbidden of FORBIDDEN_DIRECTORIES) {
    if (relativePath.startsWith(forbidden + sep) || relativePath === forbidden) {
      return {
        ...
        suggestion: getForbiddenSuggestion(forbidden, relativePath),
      };
    }
  }
 ...
 */
function getForbiddenSuggestion(forbidden, filePath) {
  const suggestions = {
 ...
 */
function getDirectorySuggestion(filePath) {
...
function validateFileStructure() {
  ...
  console.log(`${colors.red}${colors.bold}BUILD FAILED: Fix file structure violations${colors.reset}`);
  console.log(`${colors.yellow}Refer to ARCHITECTURE_RULES.md for approved structure.${colors.reset}\n`);
 ...
 */
function printApprovedStructure() {
...
${colors.bold}Usage:${colors.reset}
  node scripts/validate-file-structure.js [options]
